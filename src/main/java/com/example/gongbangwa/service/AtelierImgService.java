...
@Log4j2
public class AtelierImgService {


    @Value("${AtelierImgLocation}")
    private String atelierImgLocation;
    //DB 저장을 위해서
    ...
    public void saveAtelierImg(AtelierImg atelierImg, MultipartFile multipartFile) throws Exception{
...
//        }
        if(!StringUtils.isEmpty(oriImgName)){
            imgName = fileService.uploadFile(atelierImgLocation, oriImgName, multipartFile.getBytes());
        ...
        atelierImg.updateAtelierImg(oriImgName, imgName, imgUrl);
        atelierImgRepository.save(atelierImg);
    ...
    public void updateQuestImg (int aino, MultipartFile multipartFile) throws Exception{
        //이미지 파일 비어있는지 확인 // 이유는 우리는 화면에서 input file을 다 열어놔서
        //빈 값이 올수 있음
        if (!multipartFile.isEmpty()) {
            //이미지 가져오기 아이디를 가지고
            AtelierImg savedAtelierImg = atelierImgRepository
                    .findById(aino).orElseThrow(EntityNotFoundException::new);
            //기존 물리적 이미지파일을 삭제
            if (  !StringUtils.isEmpty(savedAtelierImg.getImgName())) {
                fileService.deleteFile(atelierImgLocation +  "/"  + savedAtelierImg.getImgName() );
            }
            // 입력받은 이미지파일의 명가져와서 fileService에 있는 물리적인 파일 저장
            String orImgName = multipartFile.getOriginalFilename();
            String imgName = fileService.
                    uploadFile(atelierImgLocation, orImgName, multipartFile.getBytes());
            // /images/item
            String imgUrl = "/images/quest/" + imgName;
            // 엔티티는 현재 영속 상태이므로 데이터를 변경하는 것만으로도
            // 변경감지기능 동작하여 트랜잭션이 끝날때 update 쿼리 실행
            savedAtelierImg.updateAtelierImg(orImgName, imgName, imgUrl);
